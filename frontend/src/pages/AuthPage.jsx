...
function Owl({ watching }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:24, zIndex:10, maxWidth:360, textAlign:'center' }}>
      <div style={{ position:'relative', display:'flex', justifyContent:'center', alignItems:'center' }}>
        {/* Glow behind Owl */}
        <div style={{ position: 'absolute', width: '220px', height: '220px', borderRadius: '50%', background: 'radial-gradient(circle, var(--accent) 0%, transparent 70%)', filter: 'blur(40px)', opacity: 0.12, zIndex: 1 }}/>

        <svg viewBox="0 0 80 80" width="160" height="160" style={{ position:'relative', zIndex:2 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
        <div style={{ whiteSpace:'nowrap', padding:'4px 12px', borderRadius:99, fontSize:11.5, fontFamily:"'Geist'", fontWeight:600, background:watching?'var(--accent-muted)':'var(--bg-elevated)', color:watching?'var(--text-accent)':'var(--text-secondary)', border:`1px solid ${watching?'var(--accent-border)':'var(--border-subtle)'}`, transition: 'all 0.2s ease', display: 'inline-block' }}>
          {watching ? 'I see you! 👀' : 'Security Mode 🙈'}
        </div>
        <h2 style={{ fontFamily: "'Geist'", fontWeight: 700, fontSize: '20px', letterSpacing: '-0.02em', color: 'var(--text-primary)', marginTop: 8 }}>
          Meet your AI career guide 🦉
        </h2>
        <p style={{ fontFamily: "'Geist'", fontSize: '13.5px', color: 'var(--text-secondary)', lineHeight: 1.6, padding: '0 12px', margin: 0 }}>
          SkillBridge secures your credentials and automates your journey to top-tier internships and full-time careers.
        </p>
      </div>
    </div>
  );
    ...
    <div className="auth-split-container">
      <style>{`
        ...
        .auth-left-panel {
          ...
          background: radial-gradient(circle at 50% 50%, var(--bg-overlay), var(--bg-base));
        ...
        /* Sleek thin scrollbar for the scrollable right panel */
      ...
      {/* Left panel: Floating Owl filling the entire left column */}
      <div className="auth-left-panel">
        <Owl watching={showPassword} />
      </div>

      {/* Right panel: Centered scrollable form card */}
      <div className="auth-right-panel">
        <div style={{ width: '100%', maxWidth: 400, margin: 'auto' }}>
          ...
          <p style={{ textAlign: 'center', fontFamily: "'Geist'", fontSize: 13, color: 'var(--text-secondary)', marginTop: 20, marginBottom: 10 }}>
